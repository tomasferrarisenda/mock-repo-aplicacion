...
import datePickerWrapTempalte = require('./date-picker.html');
import timePickerWrapTemplate = require('./time-picker.html');
...
(function () {
	...
	.controller('DateTimePickerController', ['$scope', '$element', '$attrs', '$compile', '$parse', '$document', '$timeout', '$uibPosition', 'dateFilter', 'uibDateParser', 'uiDatetimePickerConfig', '$rootScope',
		function ($scope, $element, $attrs, $compile, $parse, $document, $timeout, $uibPosition, dateFilter, uibDateParser, uiDatetimePickerConfig, $rootScope) {
			...
			this.init = function (pNgModel) {
				...
				$scope.initialPicker = angular.isDefined($attrs.initialPicker) ? $attrs.initialPicker :
				...
				if (uiDatetimePickerConfig.html5Types[$attrs.type]) {
					...
					$attrs.$observe('datetimePicker', function (value) {
						const newDateFormat = value || uiDatetimePickerConfig.dateFormat;
				...
				// datepicker element
				const datepickerEl = angular.element(popupEl.children()[0]);
				...
				// timepicker element
				const timepickerEl = angular.element(popupEl.children()[1]);
				...
				for (const key in $scope.timepickerOptions) {
				...
				ngModel.$viewChangeListeners.push(function () {
					if ($scope.timepickerOptions.min) {
						const startHour = new Date($scope.timepickerOptions.min).getHours(),
					...
					if ($scope.timepickerOptions.max) {
						const endHour = new Date($scope.timepickerOptions.max).getHours(),
				...
				function readAsFormatter(value) {
					...
					if (angular.isDate(d)) return d;

					return value;
				}

				function saveAsParser(value: any) {
					if (!value || angular.isString(value) || !angular.isDate(value))
						return value;

					if (saveAs === 'ISO') return value.toISOString();

					if (saveAs === 'json') return value.toJSON();

					if (saveAs === 'number') return value.valueOf();
			...
			$scope.dateSelection = function (dt, opt) {
				...
				if (angular.isDefined(dt)) {
					if (!$scope.date) {
						const defaultTime = angular.isDefined($attrs.defaultTime) ? $attrs.defaultTime : uiDatetimePickerConfig.defaultTime;
						const t = new Date('2001-01-01 ' + defaultTime);
				...
				var date: Date = $scope.date ? dateFilter($scope.date, dateFormat) : null;
			...
			$scope.$watch('isOpen', function (value) {
				...
				if (value) {
					...
					const position = appendToBody ? $uibPosition.offset($element) : $uibPosition.position($element);
			...
			$scope.compare = function (date1: any, date2: any): any {
				const fecha1: any = new Date(date1.getFullYear(), date1.getMonth(), date1.getDate());
				const fecha2: any = new Date(date2.getFullYear(), date2.getMonth(), date2.getDate());
			...
			$scope.$on('$destroy', function () {
				...
				watchListeners.forEach(function (a: Function) {
					a();
				});
				$popup.remove();
			...
			function documentClickBind(evt) {
				const popup = $popup[0];
				const dpContainsTarget = $element[0].contains(evt.target);

				// The popup node may not be an element node
				// In some browsers (IE only) element nodes have the 'contains' function
				const popupContainsTarget = popup.contains !== undefined && popup.contains(evt.target);
	...
	.directive('datePickerWrap', function (): any {
		return {
			restrict: 'EA',
			replace: true,
			transclude: true,
			template: datePickerWrapTempalte
		};
	})

	.directive('timePickerWrap', function (): any {
		return {
			restrict: 'EA',
			replace: true,
			transclude: true,
			template: timePickerWrapTemplate
		};
	});
