...
export default (function(){
	...
	module.init = function (module) {
		...
		module.controller('ConfiguracionDefectoEquipoTplController', ConfiguracionDefectoEquipoTplController);

		ConfiguracionDefectoEquipoTplController.$inject = ['Logger', '$state', 'ModalService', 'EquiposDataService', '$scope', '$uibModalInstance', 
		'idConfiguracionEdit', 'matriculaProfesional', '$q', '$stateParams', 'ProfesionalesDataService', 'DateUtils', 'idEquipo', 'SupportDataService', 'idTipoEquipo', 'AmbitoDataService'];

		function ConfiguracionDefectoEquipoTplController ($log, $state, ModalService, EquiposDataService, $scope, $uibModalInstance, 
			idConfiguracionEdit, matriculaProfesional, $q, $stateParams, ProfesionalesDataService, DateUtils, idEquipo, SupportDataService: ISupportDataService, idTipoEquipo,
			AmbitoDataService : IAmbitoDataService) {

			/* ------------------------------------------------ LOG ------------------------------------------------ */

			$log = $log.getInstance('ConfiguracionDefectoEquipoTplController');
			/* ----------------------------------------- API Y VARIABLESES ----------------------------------------- */
			...
			function guardar(isValid) {
				...
				vm.data.configuracionEdit.IdRecurso = idEquipo;
				vm.data.configuracionEdit.IdTipoRecurso = idTipoEquipo;
				...
				EquiposDataService.ConfiguracionGuardar(vm.data.configuracionEdit)
			...
			function activate () {
				...
				obtenerConfiguracionDto().then(function(configuracionEdit){
					...
					EquiposDataService.SucursalObtenerTodos().then(function(sucursales){
					...
					EquiposDataService.ServicioObtenerTodos().then(function(servicio){
					...
					EquiposDataService.DiaSemanaObtenerTodos().then(function(dias){
						vm.filter.dias = dias;
						if(vm.data.configuracionEdit.IdDiaSemana !== 0) vm.filter.IdDiaSemanaElegido = vm.data.configuracionEdit.IdDiaSemana;
					});

					EquiposDataService.TipoDeSemanaObtenerTodos().then(function(tipoSemana){
			...
			function obtenerConfiguracionDto(){
				...
				if(idConfiguracionEdit){
					EquiposDataService.ConfiguracionObtenerPorId(idConfiguracionEdit)
					.then(function(configuracionDto){
						def.resolve(configuracionDto);
					});
				}
				else{
					EquiposDataService.ConfiguracionObtenerNuevo()
					.then(function(configuracionDto){
						def.resolve(configuracionDto);
					});
				}
